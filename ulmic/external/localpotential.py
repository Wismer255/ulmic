...
class LocalPotential1D:
    ...
    def get_real_space_hamiltonian(self,q=0.0):
        ...
        kinetic_operator = np.zeros((nx,nx), dtype=complex)
        potential_operator = np.zeros((nx,nx), dtype=complex)
        for i in range(nx):
    ...
    def get_plane_wave_hamiltonian(self,q=0.0):
        ...
        hamilton_kinetic = np.zeros((knt,knt), dtype=complex)
        hamilton_potential = np.zeros((knt,knt), dtype=complex)
        hamilton_periodic = np.zeros((knt,knt), dtype=complex)
        potential = self.create_potential()
        for i in range(knt):
    ...
    def get_momentum_and_overlap_matrices(self,):
        ...
        def get_momentum_operator():
            ''' Three point-stencil '''
            derivative_operator = np.zeros((nx, nx), dtype=complex)
            for i in range(nx):
        ...
        def get_all_wave_functions():
            wave_functions = np.zeros((nk,nx,nb), dtype=complex)
            for k in range(nk):
        ...
        def get_momentum_matrix_elements():
            momentum = np.zeros((nk, nb, nb), dtype=complex)
            for k in range(nk):
                for i in range(nb):
        ...
        def get_overlap_matrix_elements():
            overlap = np.zeros((nk, nb, nb), dtype=complex)
            for k in range(nk):
    ...
    def save_as_medium_to_hdf5(self,output_file):
        ...
        overlap = np.zeros((nk,3,2,nb,nb), dtype=complex)
